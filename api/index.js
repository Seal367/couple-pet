...
async function updatePetStats(coupleId) {
  ...
  // 衰减速率（每分钟）：饱腹~24h归零，开心~30h归零，精力~20h归零
...
app.put('/api/me/display-name', requireAuth, async (req, res) => {
  try {
    ...
    await pool.query('UPDATE users SET display_name = $1 WHERE id = $2', [displayName.trim(), req.session.userId]);
...
app.put('/api/pet/name', requireAuth, async (req, res) => {
  try {
    ...
    await pool.query('UPDATE pets SET name = $1 WHERE couple_id = $2', [name.trim(), req.session.coupleId]);
